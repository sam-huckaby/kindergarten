...
export default function Practice() {
  ...
  return (
    <main className="flex md:px-16 pb-4 pt-24 h-full">
      <div className="flex flex-col items-center justify-between grow">
        ...
        <div className="flex flex-row justify-center items-center">
          <button className="h-full select-none p-4 rounded border border-black dark:border-white border-solid w-full text-center disabled:bg-gray-300/50 disabled:text-gray-500 dark:disabled:bg-gray-800/50 dark:disabled:text-gray-500" disabled={position < 1} onClick={prevLetter}>Prev</button>

          <div className="flex flex-col md:flex-row justify-between items-center gap-4 mx-4">
            <select value={jump} className="w-full md:w-min select-none p-4 rounded border border-solid border-black dark:border-white dark:bg-black" onChange={(e) => jumpToLetter(e.target.value)}>
              <option value="JUMP">Jump to</option>
              {
                alphabets[language].letters.map((letter) => <option key={letter} value={letter}>{letter}</option>)
              }
            </select>
            <button className="select-none p-4 rounded border border-black dark:border-white border-solid w-full text-center disabled:bg-gray-300/50 disabled:text-gray-500 dark:disabled:bg-gray-800/50 dark:disabled:text-gray-500" onClick={jumpToRandom}>Random</button>
          </div>


          <button className="h-full select-none p-4 rounded border border-black dark:border-white border-solid w-full text-center disabled:bg-gray-300/50 disabled:text-gray-500 dark:disabled:bg-gray-800/50 dark:disabled:text-gray-500" disabled={position > alphabets[language].letters.length - 2} onClick={nextLetter}>Next</button>
        </div>
      </div>
    </main>
  )
